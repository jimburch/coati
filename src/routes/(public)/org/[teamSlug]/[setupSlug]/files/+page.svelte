<script lang="ts">
	...
	const basePath = $derived(`/org/${setup.teamSlug}/${setup.slug}/files`);
</script>

<svelte:head>
	<title>Files - {setup.name} - Coati</title>
</svelte:head>
...
<div class="mx-auto max-w-7xl px-4 py-6 lg:py-8">
	...
	<nav class="mb-4 flex items-center gap-1.5 text-sm text-muted-foreground lg:mb-6">
		<a href="/org/{setup.teamSlug}" class="hover:text-foreground hover:underline">
			{setup.teamName}
		</a>
		<ChevronRight class="size-3.5" />
		<a href="/org/{setup.teamSlug}/{setup.slug}" class="hover:text-foreground hover:underline">
			{setup.slug}
		</a>
		<ChevronRight class="size-3.5" />
